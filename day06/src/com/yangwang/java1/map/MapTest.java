package com.yangwang.java1.map;
 ...
 *             /----LinkedHashaMap:保证在遍历map元素时，可以按照添加的顺序实现遍历。
 *                  原因：在原有的HashMap底层结构基础上，添加了一对指针。指向前一个和后一个元素。
 *                  对于频繁的遍历操作，此类执行效率高于HashMap。
 *         /----TreeMap:保证按照添加的Key-value对进行排序，实现遍历排序。此时考虑key的自然排序
 *                      或定制排序，底层使用红黑树
 ...
 *
 *   二、Map结构的理解：
 *     Map中的key：无序的、不可重复的，使用Set存储所有的key --->key所在的类要重写equals()和hashCode()(以HashMao为例)
 ...
 *        当数组的某一个索引位置上的元素以聊表形式存在的数据个数 > 8 且当前数组的长度 >64时，
 *        此时此索引位置上的所有数据改成使用红黑树存储。
 *
...
public class MapTest {
    ...
    public void test1() {
        ...
        //元素查询的操作：
        //Object get(Object key):获取指定key对应的value
        String s = map.get("jim");
        System.out.println(s);

        //boolean containsKey(Object key):是否包含指定的key
        boolean b = map.containsKey("jim");
        System.out.println(b);

        //boolean containsValue(Object value):是否包含指定的value
        boolean value = map.containsValue("tianjin");
